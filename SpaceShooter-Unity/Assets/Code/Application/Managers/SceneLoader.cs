...
namespace Code.Application.Managers
{
    public class SceneLoader
    {
        ...
        private const string MAINMENU_SCENE_NAME = "MainMenuScene";
        ...
        public UniTask  LoadSceneUniTask(EScene scene, Action callback = null)
        {
            var nameScene = GetNameScene(scene);

            return LoadScenes(nameScene, callback);
        }
        
        public async UniTask UnloadSceneAndLoadLoading(EScene sceneToUnload)
        {
            if (!IsSceneLoaded(LOADING_SCENE_NAME))
            {
                await SceneManager.LoadSceneAsync(LOADING_SCENE_NAME, LoadSceneMode.Additive).ToUniTask();
            }

            await _loadingBarAdapter.LoadingBarKeeper
                .RunDependentLoadingBarAnimation(DependLoadingType.GAME_SCENE_LOAD_START);

            var targetScene = GetNameScene(sceneToUnload);
            if (IsSceneLoaded(targetScene))
            {
                await SceneManager.UnloadSceneAsync(targetScene).ToUniTask();
            }
        }

        public async UniTask LoadSceneAndUnloadLoading(EScene sceneToLoad, Action sceneLoaded = null)
        {
            
            var sceneName = GetNameScene(sceneToLoad);
            ...
            // выгружаем LoadingScene только если она действительно активна
            if (IsSceneLoaded(LOADING_SCENE_NAME))
            {
                await SceneManager.UnloadSceneAsync(LOADING_SCENE_NAME).ToUniTask();
            }
